document.addEventListener("DOMContentLoaded", function () {
    ...
    const postUrl = `https://3.38.95.210/api/posts/${postId}/`;
    ...
    fetch(postUrl)
        ...
        .then(data => {
            ...
            if (data.image) {
                const imageUrl = `https://3.38.95.210${data.image}`;  // 서버 URL에 /media 경로 추가
    ...
    document.getElementById('comment-form').addEventListener('submit', function (e) {
        ...
        fetch(`https://3.38.95.210/api/posts/${postId}/comment/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`  // JWT 토큰 추가
    ...
    function editComment(commentId, newContent) {
        const editUrl = `https://3.38.95.210/api/posts/comment/${commentId}`;  // URL에 commentId 추가
    ...
    function deleteComment(commentId) {
        const deleteUrl = `https://3.38.95.210/api/posts/comment/${commentId}`;  // URL에 commentId 추가
